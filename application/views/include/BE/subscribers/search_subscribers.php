  ...
  <li>Manage</li>
    ...
    <div class="row">
            ...
            <table class="table table-striped table-hover table-bordered">
                ...
                <?php if ($search_subscribers->num_rows > 0) { ?>
                  <?php foreach ($search_subscribers->result() as $data) { ?>
                        <tr>
                          <td>
                            ...
                            <td>
								...
								if ($data->sub_status == 1) {
										$status = anchor('subscribers/status_subscribers/' . $data->sub_status . '/' . $data->sub_id, 'Published', 'title="Published"');
								} else if ($data->sub_status == 0) {
										$status = anchor('subscribers/status_subscribers/' . $data->sub_status . '/' . $data->sub_id , 'Unpublish', 'title="Unpublish"');
								}
                                echo
                                anchor('subscribers/view_subscribers/'.$data->sub_id.'/'.$uri, 'view') . '|' .
                                anchor('subscribers/edit_subscribers/'.$data->sub_id.'/'.$uri, 'edit') . '|' .
                                anchor('subscribers/deleteSubscriberById/' . $data->sub_id . '/' . $uri, 'delete', 'title = "Delete" onclick="return confirm(\'Are you sure want to delete this record?\');" data-toggle="tooltip" id="tooltip"').'|'.$status; 
                                ?>
                            </td>
						 </tr>
                    <?php } ?><!-- -->
