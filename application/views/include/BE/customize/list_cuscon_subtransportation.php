...
<table class="table table-striped table-hover table-bordered" style="font-size: 12px;">
    <thead>
    <tr>
        <th><input type="checkbox" name="checkbox_all_cuscon_tps" data-id=".tbl_bodytps<?php echo $tpsID ?>" class="checkbox_all_cuson_tps check_checkbox" checked="true"></th>
        <th>Name</th>
        <th>From Date</th>
        <th>To Date</th>
        <th>Purchase($)</th>
    ...
    <tbody class="sub_tps_cuscon_body tbl_bodytps<?php echo $tpsID; ?>">
    	<?php 
    		if(isset($sub_transport)){
    			foreach ($sub_transport as $subtps) {
		?>
				<tr class="real_sub_tps_cuscon remove<?php echo $subtps["tp_id"]; ?>">
					...
					<td><span class="icon-list-alt cusconadd clickDetailsubtps" data-url="<?php echo base_url(); ?>customize/subtpsdetail/<?php echo $tpsID; ?>/<?php echo $this->uri->segment(3); ?>/<?php echo $subtps["tp_id"]; ?>" data-toggle='modal' data-target='.modalDetials' title="Details"></span></td>
				</tr>
		<?php
    			}
    	    }
