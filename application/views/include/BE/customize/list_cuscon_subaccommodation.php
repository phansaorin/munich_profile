...
<table class="table table-striped table-hover table-bordered" style="font-size: 12px;">
    <thead>
    <tr>
        <th><input type="checkbox" name="checkbox_all_cuscon_acc" data-id=".tbl_bodyacc<?php echo $accID; ?>" class="checkbox_all_cuscon_acc check_checkbox" checked="true"></th>
        <th>Name</th>
        <th>From Date</th>
        <th>To Date</th>
        <th>Purchase($)</th>
    ...
    <tbody class="sub_acc_cuscon_body tbl_bodyacc<?php echo $accID; ?>">
    	<?php 
    		if(isset($sub_accommodation)){
    			foreach ($sub_accommodation as $subacc) {
		?>
				<tr class="real_sub_acc_cuscon remove<?php echo $subacc["acc_id"]; ?>">
					...
					<td><span class="icon-list-alt cusconadd clickDetailsubacc" data-url="<?php echo base_url(); ?>customize/subaccdetail/<?php echo $accID; ?>/<?php echo $this->uri->segment(3); ?>/<?php echo $subacc["acc_id"]; ?>" data-toggle='modal' data-target='.modalDetials' title="Details"></span></td>
				</tr>
		<?php
    			}
    	    }
