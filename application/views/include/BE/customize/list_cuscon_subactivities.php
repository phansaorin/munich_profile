...
<table class="table table-striped table-hover table-bordered" style="font-size: 12px;">
    <thead>
    <tr>
        <th><input type="checkbox" name="checkbox_all_cuscon_act" data-id=".tbl_bodyact<?php echo $actID; ?>" class="checkbox_all_cuscon_act check_checkbox" checked="true"></th>
        <th>Name</th>
        <th>From Date</th>
        <th>To Date</th>
        <th>Purchase($)</th>
    ...
    <tbody class="sub_act_cuscon_body tbl_bodyact<?php echo $actID; ?>">
    	<?php 
    		if(isset($sub_activites)){
    			foreach ($sub_activites as $subact) {
		?>
				<tr class="real_sub_act_cuscon remove<?php echo $subact["act_id"]; ?>">
					...
					<td><span class="icon-list-alt cusconadd clickDetailsubact" data-url="<?php echo base_url(); ?>customize/subactdetail/<?php echo $actID; ?>/<?php echo $this->uri->segment(3); ?>/<?php echo $subact["act_id"]; ?>" data-toggle='modal' data-target='.modalDetials' title="Details"></span></td>
				</tr>
		<?php
    			}
    	    }
