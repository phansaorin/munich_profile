<?php
//Additional values
$pdfFileName = 'export_e-tichet.pdf';
tcpdf();
...
?>
        ...
        foreach ($eticket_customize as $value):
            $passengerWith = unserialize($value->pbk_pass_come_with);
     ?>
     <?php 
            $content = '<div style="border: 1px solid black; height:30px;">'.$value->cuscon_name.nbs(140).'Price:'.$value->bk_pay_price.'$'.'</div>';
     ...
     $getMainActivities = $value->cuscon_activities;
 ...
 <?php                            
    ...
    $content .='<div style="background-color:gray; font-size:11px;color:#ffffff;">Acommodation</div>';
            $getAccommodation = $value->cuscon_accomodation; 
    ...
    $content .='<div style="background-color:gray; font-size:11px; color:#ffffff;">Transportation</div>';
      $getTransportation = $value->cuscon_transportation;
